...
export const activateExtension = async (req, res) => {
  try {
    ...
    await logActivity({
      ...
      domain: null,
      message: `✅ Extension connected: ${child.name}'s SproutSafe extension is now active`,
    });
...
export const disconnectExtension = async (req, res) => {
  try {
    ...
    await logActivity({
      ...
      domain: null,
      message: `❌ Extension disconnected: ${child.name}'s SproutSafe extension has been turned off`,
    });
...
export const tamperAlertFailed = async (req, res) => {
  try {
    res.status(200).json({ message: "Tamper alert failed recorded" });
  } catch (error) {
