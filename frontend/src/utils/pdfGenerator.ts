...
import type { MonitoredUrl, IncognitoAlert } from '@/types/models';
...
export const generatePDFReport = async (
    ...
    // Header
    doc.setFontSize(22);
    doc.setTextColor(34, 197, 94); // CyberNest Green
    doc.text("CyberNest Activity Report", 105, 20, { align: "center" });
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(`Page ${i} of ${pageCount} - CyberNest Report`, 105, 290, { align: "center" });
    }

    doc.save(`CyberNest_Report_${childName.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`);
};
