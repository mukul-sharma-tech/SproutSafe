...
export function AddChildModal({ open, onClose, onChildAdded }: AddChildModalProps) {
  ...
  const handleAddChild = async () => {
    ...
    setIsLoading(true);
    try {
      const email = `c${Date.now()}_${slug(childName)}@CyberNest.local`;
      const { token } = await addChild({ name: childName.trim(), email });
      setGeneratedToken(token);

      try {
        await navigator.clipboard.writeText(token);
        toast.success("Token copied to clipboard. Paste it in the CyberNest extension.");
      } catch {
          ...
          {step === "success" && generatedToken && (
            <div className="space-y-6 text-center">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">
                  Token has been copied. Paste it in the CyberNest extension to activate.
                </p>
              </div>
